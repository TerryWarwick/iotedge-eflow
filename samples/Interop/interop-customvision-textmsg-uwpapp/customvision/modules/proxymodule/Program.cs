    ...
    class Program
    {
        ...
        static async Task<MessageResponse> ForwardMessageToLeafDevice(Message message, object userContext)
        {
            try
            {
                ...
                Logger.Log($"{UtcDateTime} ForwardMessageToLeafDevice: Received message={receivedMessage} from deviceId={deviceId}.");
            ...
            {
                Logger.Log($"ForwardMessageToLeafDevice got exception {ex.InnerException}", LogSeverity.Error);
            }
